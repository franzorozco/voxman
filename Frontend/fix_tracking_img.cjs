...
const fixImages = (filePath) => {
    ...
    const regex = /const\s+colorImg\s*=\s*product\?\.attribute_value_images\?\.find\(img\s*=>\s*variantAttrIds\.includes\(img\.attribute_value_id\)\);[\s\S]*?if\s*\(!imageUrl\.startsWith\('http'\)\)\s*\{\s*imageUrl\s*=\s*`\$\{import\.meta\.env\.VITE_API_URL\?\.replace\('\/api\/v1',\s*''\)\s*\|\|\s*import\.meta\.env\.VITE_API_URL\?\.replace\('\/api',\s*''\)\s*\|\|\s*API_BASE_URL\}\$\{imageUrl\}`;\s*\}/g;

    const replaceStr = `const colorImgs = product?.attribute_value_images?.filter(img => variantAttrIds.includes(img.attribute_value_id)) || [];
...
fixImages('e:/Trabajo/Sistema de VOXman/Sistema/Frontend/src/pages/public/Tracking/Tracking.jsx');
