...
const fixImages = (filePath) => {
    ...
    const regex = /let\s+imgUrl\s*=\s*item\.variant\?\.variant_images\?\.\[0\]\?\.url;[\s\S]*?const\s+finalImgUrl\s*=\s*imgUrl\s*\?\s*\(imgUrl\.startsWith\("http"\)\s*\?\s*imgUrl\s*:\s*`\$\{API_BASE_URL\}\$\{imgUrl\}`\)\s*:\s*"\/placeholder\.png";/g;

    const replaceStr = `let imgUrl = item.variant?.variant_images?.[0]?.url || item.variant?.variant_images?.[0]?.image_path;
                              if (!imgUrl) {
