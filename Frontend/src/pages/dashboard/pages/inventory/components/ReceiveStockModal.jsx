      ...
      <div className="modal-content" style={{ background: 'var(--bg-card)', borderRadius: '16px', width: '90%', maxWidth: '600px', boxShadow: '0 10px 30px rgba(0,0,0,0.2)', overflow: 'hidden', display: 'flex', flexDirection: 'column', maxHeight: '90vh' }}>
          ...
          {selectedProduct && (
            <div style={{ marginBottom: '25px', animation: 'fadeIn 0.3s ease' }}>
              <label style={{ display: 'block', marginBottom: '12px', fontSize: '13px', fontWeight: 600, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                3. Seleccionar Variante a Ingresar
              </label>
                  ...
                  return (
                    <div 
                      ...
                      style={{ 
                        ...
                        background: isSelected ? 'var(--primary-color-alpha)' : 'var(--bg-input)', 
                        cursor: 'pointer', 
                        transition: 'all 0.2s',
                        display: 'flex',
                    ...
                    >
                      <div style={{ width: '40px', height: '40px', borderRadius: '8px', overflow: 'hidden', background: 'var(--bg-main)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                         {variant.variant_images && variant.variant_images.length > 0 ? (
                           <img src={`${API_BASE_URL}${variant.variant_images[0].url}`} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                         ) : (
                      ...
                      <div style={{ fontSize: '11px', color: 'var(--text-muted)', wordBreak: 'break-all' }}>{variant.sku}</div>
                    </div>
                  );
                })}
        ...
        <div style={{ padding: '20px', borderTop: '1px solid var(--border-color)', display: 'flex', justifyContent: 'flex-end', gap: '12px', background: 'var(--bg-main)' }}>
          ...
          <button
            ...
            disabled={loading || !selectedVariant || quantity <= 0 || !selectedBranchId}
            style={{ padding: '12px 24px', borderRadius: '10px', border: 'none', background: 'var(--color-primary)', color: 'var(--color-primary-text)', cursor: 'pointer', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '8px', opacity: (loading || !selectedVariant || quantity <= 0 || !selectedBranchId) ? 0.5 : 1 }}
          >
